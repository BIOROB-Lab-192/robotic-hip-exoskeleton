...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (emg_Original_M->Timing.clockTick0+
    emg_Original_M->Timing.clockTickH0* 4294967296.0);
  emg_Original_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(emg_Original_M, 0);
  ...
  emg_Original_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(emg_Original_M->extModeInfo, &rtmGetTFinal
    (emg_Original_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(emg_Original_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(emg_Original_M);
