...
Hertz = 25
total_runtime = 2 # RUNTIME IN SECONDS
...
class ExoGUI(object):
    def __init__(self):
        ...
        self.Ant = kqio.AntCH('com3')  # "/dev/ttyAMA0"
    ...
    def CtrlLoop(self):
        ...
        TorFreq = 0.5
        TorMiddle = 0.5 * kqio.TOR_MAX_VALUE
        TorRange = 0.25 * kqio.TOR_MAX_VALUE
        TorPhase = 0
        TorRef = 0
        ...
        while AntConnected:
            if UpdateState == 1:
                ...
                TorPhase += TorFreq * self.Ant.Ctrldt
                ...
                if TorPhase > 1:
                    TorPhase -= 1
                TorRef = TorRange * np.sin(TorPhase * np.pi * 2)

                # self.Ant.Cmd.Loop_L = kqio.PLACE_LOOP
                # self.Ant.Cmd.Loop_R = kqio.PLACE_LOOP
                
                # Torque Loop limit does not work 
                self.Ant.Cmd.Loop_L = kqio.TOR_LOOP
                self.Ant.Cmd.Loop_R = kqio.TOR_LOOP
                ...
                self.Ant.Cmd.Value_L = (TorMiddle + TorRef) * SmoothRate + StartHipAngle_L * (1 - SmoothRate)
                self.Ant.Cmd.Value_R = (TorMiddle - TorRef) * SmoothRate + StartHipAngle_R * (1 - SmoothRate)
